import { Comment, CreateEvent, EventProps, Like, Reply, UpdateEvent } from '@/types/event'
...
export class EventRepository implements EventRepositorySource {
   ...
   constructor(database: Db) {
      this.eventCollection = database.collection<EventProps>('events')
   }
   ...
   async getAll(): Promise<EventProps[]> {
      ...
      // find().project<EventPropsBasicProps>()
      try {
         const events: EventProps[] = await this.eventCollection
            .aggregate<EventProps>([
               {
                  $sort: { date: 1 },
               },
               {
   ...
   async getById(eventId: string): Promise<EventProps | null> {
      try {
         const event: EventProps | null = await this.eventCollection.findOne<EventProps>({
   ...
   async getByCategory(
      ...
      try {
         ...
         const event = await this.eventCollection
            // .aggregate<EventProps>([
            .find<EventProps>({ category: category })
   ...
   async create(event: CreateEvent): Promise<InsertOneResult<EventProps>> {
      try {
         const newEvent: InsertOneResult<EventProps> = await this.eventCollection.insertOne({
            _id: new ObjectId(),
   ...
   async update(event: UpdateEvent): Promise<WithId<EventProps> | null> {
      try {
         const updatedEvent: WithId<EventProps> | null =
   ...
   async toggleEventLike(
      ...
      try {
         const toggleLike: WithId<EventProps> | null = await this.eventCollection.findOneAndUpdate(
            {
               _id: new ObjectId(eventId),
            },
            {
   ...
   async addComment(
      ...
      try {
         const newComment: WithId<EventProps> | null = await this.eventCollection.findOneAndUpdate(
            {
               _id: new ObjectId(eventId),
            },
   ...
   async addReplyToComment(
      ...
      try {
         const addReply: WithId<EventProps> | null = await this.eventCollection.findOneAndUpdate(
            {
               _id: new ObjectId(eventId),
               'comments._id': new ObjectId(commentId),
            },
   ...
   async isViperParticipant(eventId: string, viperId: string): Promise<boolean> {
      try {
         ...
         throw new Error(`Repository Error: Failed to check if Viper is Participating, ${error}`)
      }
   }
